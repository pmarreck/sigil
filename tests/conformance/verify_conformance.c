/* Conformance consumer for sigil_verify — the raw Ed25519 primitive.
 *
 ...
 *
 * The vectors are RFC 8032 §7.1 TEST 1 — an oracle written by the IETF rather
 * than by us, so it cannot agree with our own mistakes.
 */
#include <stdio.h>
#include <string.h>
#include "sigil.h"
...
int main(void) {
	...
	printf("sigil_verify conformance (RFC 8032 vectors, stock cc)\n");

	check(sigil_public_key_len() == sizeof pk, "public key length is 32");
	check(sigil_signature_len() == sizeof sig, "signature length is 64");

	if (unhex(PK_HEX, pk, sizeof pk) != 0 || unhex(SIG_HEX, sig, sizeof sig) != 0) {
		printf("  FAIL could not decode the RFC vectors\n");
		return 1;
	}

	/* --- Sensitivity: the standard's own signature must verify. --- */
	check(sigil_verify((const unsigned char *)"", 0, sig, pk) == SIGIL_OK,
	      "RFC 8032 TEST 1 signature verifies over the empty message");

	/* --- Specificity: every corruption must be REJECTED. A verifier that
	 *     accepts everything passes the test above; only these can fail it. --- */
	{
		unsigned char bad[64];
		memcpy(bad, sig, sizeof bad);
		bad[0] ^= 0x01;
		check(sigil_verify((const unsigned char *)"", 0, bad, pk) != SIGIL_OK,
	...
	}
	{
		...
		memcpy(badpk, pk, sizeof badpk);
		badpk[0] ^= 0x01;
		check(sigil_verify((const unsigned char *)"", 0, sig, badpk) != SIGIL_OK,
	...
	check(sigil_verify((const unsigned char *)"x", 1, sig, pk) != SIGIL_OK,
	      "a different message is rejected");

	/* --- NULL handling: the header promises only NULL is an argument error. --- */
	check(sigil_verify(NULL, 0, sig, pk) == SIGIL_ERR_NULL_ARGUMENT,
	      "NULL payload is an argument error, not a crash");
	check(sigil_verify((const unsigned char *)"", 0, NULL, pk) == SIGIL_ERR_NULL_ARGUMENT,
	      "NULL signature is an argument error");
	check(sigil_verify((const unsigned char *)"", 0, sig, NULL) == SIGIL_ERR_NULL_ARGUMENT,
